...
import de.jmizv.jiexplorer.filetask.FileTaskListener;
import de.jmizv.jiexplorer.filetask.ScanFileTask;
	...
	public void update(final JIObservable list, final Object obj) {
		...
		if (obj instanceof String ) {
			if(((String)obj).equals(JIObservable.SECTION_CHANGED)) {
								...
								public void run() {
									try {
										...
										for (final JITreeNode node: nodes) {
											// Get images for this directory only
				...
				// User requested generate all image thumbnails
					...
					final Thread w = new Thread() {
						...
						public void run() {
							try {
								...
								final Vector<File> files = new Vector<File>();
								for (final JITreeNode node: nodes) {
									// get images for this directy and all its children
