...
import de.jmizv.jiexplorer.gui.dnd.ListTransferHandler;
...
final public class JIThumbnailList extends JList implements JIObservable {
	...
	//private ListTransferHandler scrollMode;
	...
	public final String getToolTipText(final MouseEvent evt) {
		...
		return "<html>" + ((DiskObject) item).getName() + "<br>" + "Size: " + ((DiskObject) item).getSize() + "<br>"
		+ ((((DiskObject) item).getDimension().width == -1) ? "" : "Dimension: " + ((DiskObject) item).getDim() + "<br>") + "Date: "
		+ ((DiskObject) item).getShortDate() + "</html>";
	}
	...
	protected final void listValueChanged(final ListSelectionEvent e) {
		//if (!((JIListModel)this.getModel()).isLoading()) {
			//log.debug("JIFileTable::valueChanged - e.getFirstIndex() = "+e.getFirstIndex()+"  e.getLastIndex() = "+e.getLastIndex());
			//log.debug("JIFileTable::valueChanged - source = "+e.getSource().getClass().getName());
			//log.debug("JIFileTable::valueChanged - "+e.toString());
		...
		if (getSelectionModel().isSelectedIndex(index)) {
			...
			if (SwingUtilities.isLeftMouseButton(e) && (e.getClickCount() == 2)) {
				...
				if ((index == this.lastClickIndex) && (this.lastClickTime > -1)) {
					if ((clickTime-this.lastClickTime > 400) && (clickTime-this.lastClickTime < 2500) &&
					   ((this.getSelectedIndices() == null) || (getSelectedIndices().length == 1))) {
	...
	public void notifyObservers(final Object arg) {
		...
		for (int i = arrLocal.length-1; i>=0; i--) {
			//log.debug("JIThumbnailList::notifyObservers - Observer = "+arrLocal[i].getClass().getName()+" arg = "+arg);
